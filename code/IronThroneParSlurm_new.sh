...
verbose=0
skip_shuf=0
partition=pe2 #Shouguo Gao pe2
pcr_read_threshold=0.5
skip_iron_throne=0
levenshtein_distance=0.1
low_mem=0
threads=22 #Shouguo Gao $(nproc)
...
#comment by Shouguo Gao SBATCH --partition=${partition}
#SBATCH --mem=10g
#SBATCH --output=%j.log
...
EOF
sbatch --cpus-per-task=22 --time=10:00:00 ../IronThroneCustomInput.sh ##Add cpucpus per task by Shouguo
...
#comments by Shouguo #SBATCH --partition=${partition}
#SBATCH --mem=10g
#SBATCH --output=%j.log
...
EOF
##Add cpucpus per task by Shouguo
sbatch --cpus-per-task=22 CombineIronThronePar.sh
